...
def run_fixtures() -> dict:
    ...
    for fix in data["fixtures"]:
        try:
            verdict = validate_explanation(fix["fen"], fix["analysis"], fix.get("fen_before"))
        ...
        if missing_moves or missing_captures:
            coverage_ok = False
            print(f"!! extractor missed claims in {fix['id']}: moves={missing_moves} captures={missing_captures}")
...
def run_log_replay() -> dict:
    ...
    for line in LOG_PATH.read_text().splitlines():
        ...
        stats = by_model.setdefault(key, {"calls": 0, "scored": 0, "n_total": 0, "n_passed": 0, "illegal": 0, "false_captures": 0, "parse_errors": 0})
        stats["calls"] += 1
        ...
        if not analysis or not fen_after:
            continue
        try:
            verdict = validate_explanation(fen_after, analysis, fen_before)
        except ValueError:
            continue
    ...
    for key, stats in by_model.items():
        ...
        stats["rejection_rate"] = round((stats["illegal"] + stats["false_captures"]) / claims, 3) if claims else None
...
def main() -> int:
    ...
    for v in fixtures["per_fixture"]:
        g = v.get("grounded")
        print(f"  {v['id']:24s} grounded={str(g):5s} illegal_moves={v.get('illegal_moves')} false_captures={v.get('false_captures')}")
